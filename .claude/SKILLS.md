...
```bash
python .claude/skills/query-sparql/query.py 'PREFIX op: <https://open-pulse.epfl.ch/ontology#>
  SELECT (MAX(?s) AS ?stars) WHERE { <https://github.com/Biohub/esm> op:githubRepoStars ?s }'

python .claude/skills/query-neo4j/query.py "MATCH (r:Repo {full_name:'https://github.com/Biohub/esm'})-[rel]-(n)
...
python .claude/skills/query-chaoss/query.py repo Biohub esm
python .claude/skills/op-collections/query.py rows github_repos --q "Biohub/esm" --size 1
python .claude/skills/op-search/query.py search github_repos "protein language model" --top-k 3
```
