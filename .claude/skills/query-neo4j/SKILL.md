...
No `.git` suffix. Cross-store: SPARQL uses the same repo URL as its subject IRI; CHAOSS and collections take `owner` + `repo` separately; OpenSearch's `repo_name` is a clone URL whose `.git` suffix varies. See `.claude/SKILLS.md` §12.
