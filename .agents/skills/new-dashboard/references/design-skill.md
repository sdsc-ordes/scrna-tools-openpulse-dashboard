...
**design skill** (`.agents/skills/<brand>/`) that the scaffold builds against — instead of a
...
This is the *agent-facing* version of `.agents/SKILLS.md` §11 (the human how-to). Read §11
...
- **Brand name / slug** — becomes the skill directory `.agents/skills/<slug>/`.
...
Two files minimum (`.agents/SKILLS.md` §11 shows the full-brand layout; a custom theme can
stay minimal):
...
2. **Update the active-skill line** in `AGENTS.md` → *Design system* to name the new skill.
3. **Run `node tools/sync-agents.mjs`** so `.agents/` + `AGENTS.md` regenerate (CI fails on
   drift). This is required for every `.agents/` edit, the new skill included.
